...
function groupByColumn(items: SourceItemProps[], t: (key: string) => string) {
  const techColumn = t("columns.tech")
  const uncategorized = t("columns.uncategorized")
  return items.reduce((acc, item) => {
    ...
    if (m.column === techColumn) return -1
    if (n.column === techColumn) return 1
...
export function SearchBar() {
  ...
  console.log("SearchBar sources:", sources)
    ...
    () =>
      groupByColumn(typeSafeObjectEntries(sources)
        ...
        .map(([k, source]) => ({
          id: k,
          title: source.title,
          column: source.column ? t(`columns.${source.column}`) : t("columns.uncategorized"),
          name: source.name,
